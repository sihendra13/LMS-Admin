...
const CustomSelect = ({ value, onChange, options, disabled = false, align = 'left', minWidth = '160px' }) => {
  ...
  return (
    <div ref={ref} style={{ position: 'relative', width: '100%', minWidth }}>
      ...
      {open && !disabled && (
        <div className="custom-scrollbar" style={{ position: 'absolute', top: '42px', [align === 'right' ? 'right' : 'left']: 0, zIndex: 200, background: '#fff', border: '1px solid var(--border)', borderRadius: '8px', boxShadow: '0 4px 16px rgba(0,0,0,0.12)', minWidth: '100%', maxHeight: '320px', overflowY: 'auto', scrollbarWidth: 'thin', scrollbarColor: '#94a3b8 #f1f5f9' }}>
          {options.map(o => (
            <div
              key={o.value}
